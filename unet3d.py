...
#     input_tensor = torch.randn(1, 1, 192, 256, 256).to(device)  # Move input to the same device

#     summary(model=model, input_size=(1, 192, 128, 128), batch_size=1, device=device.type)
...
class UpConv3DBlock(nn.Module):
    ...
    def forward(self, input, residual=None):
        ...
        if residual is not None:
            # Resize output to match residual size using interpolation
            out = F.interpolate(out, size=residual.shape[2:], mode='trilinear', align_corners=False)
            
            # Concatenate residual and upsampled tensor
...
if __name__ == '__main__':
    ...
    input_tensor = torch.randn(1, 1, 256, 298, 256).to(device)  # Example variable-sized input
